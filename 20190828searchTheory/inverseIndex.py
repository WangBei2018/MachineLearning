...
if __name__ == '__main__':
    ...
    for doc_id, text in documents.items():
        ...
        doc_index = inverted_index(text)                #求每个文档中每个字词所在位置,并添加到分词结果数组中
        inverted_index_add(inverted, doc_id, doc_index) #再将文档名称添加

    for word, doc_locations in inverted.items():
        print (word, doc_locations)                     #输出倒排索引
    ...
    # queries = ['创始人', '谷歌', '加盟']
    queries = words
    docnum = []                         #记录每个词出现的字典的个数
    for query in queries:
        result_docs = search(inverted, query)
        print("Search for '%s': %s" % (query, u','.join(result_docs.keys())))   # query为搜索的字符串数组，result_docs.keys()为所在的文档名称
        #print('',len(result_docs.keys()))
    ...
    tf = [[0 for i in range(len(words))] for i in range(len(spilt_str))]
    idf = [[0 for i in range(len(words))] for i in range(len(spilt_str))]
    # print(D)
    #print('docnum:',docnum)
    tf_idf = [[0 for i in range(len(words))] for i in range(len(spilt_str))]
    for i in range(0,len(spilt_str)):
        for j in range(0,len(words)):
            tf[i][j] = D[i,j]/len(corpus[i])
            # print(words[j])
            # print('D--',D[i,j])
            # print('len(corpus[i])',len(corpus[i]))
            # print('tf--',tf[i][j])
            # print('len(spilt_str)--',len(spilt_str))
            # print('1+docnum[i]:',1+docnum[j])
            idf[i][j] = math.log(len(spilt_str)/(1+docnum[j]),10)
            # print('tf[i][j]:',tf[i][j])
            # print('idf[i][j]',idf[i][j])
